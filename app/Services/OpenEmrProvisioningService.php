...
class OpenEmrProvisioningService
{
    ...
    protected function applyTenantOverrides(string $dbName, Subscription $subscription, ?string $sitePath = null): void
    {
        ...
        // Apply demographic layout configuration, location lists (Country, State, County) & dependent cascading dropdowns
    ...
    protected function createFirstAdminUserAndMapAcl(string $dbName, Subscription $subscription, ?string $sitePath = null): void
    {
        ...
        $adminPassHash = password_hash($adminDefaultPass, PASSWORD_DEFAULT);
        $adminId = $pdo->query("SELECT id FROM users WHERE username = 'admin'")->fetchColumn();

        if (!$adminId) {
            $stmtAdmin = $pdo->prepare("INSERT INTO users (username, password, fname, lname, authorized, active, calendar, cal_ui, facility_id, info, main_menu_role, date_created, last_updated) 
                VALUES ('admin', 'NoLongerUsed', 'Billy', 'Smith', 1, 1, 1, 3, 3, 'Administrator', 'standard', NOW(), NOW())");
            $stmtAdmin->execute();
            $adminId = (int) $pdo->lastInsertId();
        } else {
            $adminId = (int) $adminId;
            $pdo->exec("UPDATE users SET fname = 'Billy', lname = 'Smith', authorized = 1, active = 1, calendar = 1, cal_ui = 3, facility_id = 3, main_menu_role = 'standard', last_updated = NOW() WHERE id = {$adminId}");
        }
